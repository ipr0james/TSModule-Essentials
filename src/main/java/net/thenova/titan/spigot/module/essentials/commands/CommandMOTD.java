 ...
 */
public final class CommandMOTD extends SpigotCommand<SpigotSender> implements CommandPermission<SpigotSender> {
    ...
    public void execute(SpigotSender sender, CommandContext context) {
        MessageHandler.INSTANCE.message(EssentialsHandler.INSTANCE.getMOTD())
                .placeholder(new PlayerPlaceholder(sender.getSender().getName()))
                .send(sender);
    }

    @Override
    public boolean hasPermission(SpigotSender sender) {
        return sender.getSender().hasPermission("titan.command.motd");
    }
}

 ...
 */
public final class CommandRules extends SpigotCommand<SpigotSender> implements CommandPermission<SpigotSender> {
    ...
    public void execute(SpigotSender sender, CommandContext context) {
        MessageHandler.INSTANCE.message(EssentialsHandler.INSTANCE.getRules())
                .send(sender);
    }

    @Override
    public boolean hasPermission(SpigotSender sender) {
        return sender.getSender().hasPermission("titan.command.rules");
    }
}
